            ...
            client.map((item,index)=>(
              <div data-aos="zoom-in" data-aos-delay="200" key ={index} className="bg-white dark:bg-gray-900 hover:bg-red-100 cursor-pointer p-12 flex flex-col justify-center items-center gap-6 rounded-xl w-full">
                <div className="flex justify-start items-center w-full gap-4">
                  <img src ={item.image}   className="w-[70px] transform hover:scale-110 transition-transform duration-300"/>=
                  <div className="flex flex-col justify-center items-start gap-1">
                    <h1 className="text-xl text-white font-semibold dark:text-white">{item.name}</h1>
                    <h1 className="text-slate-300 ">{item.text}</h1>
                  </div>
                </div>
                <p className="text-md text-justify text-slate-300">{item.feedback}</p>
